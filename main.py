...
ext = tuple([".jpg", ".png", ".jpeg", ".wepb", ".gif", ".bmp", ".heic", ".pdf", ".tif", ".webp"])
...
async def cdata(c, q):
    ...
    if data == "home":
        ...
        exp = int(num) if num != "0" else None
        await q.answer(wait)
...
@Img.on_message(filters.private & (filters.photo | filters.sticker | filters.document | filters.animation))
async def getimglink(c, m):
    if not Var.API:
        return await m.reply_text(Tr.ERR_TEXT, quote=True)
    if m.document and not m.document.file_name.endswith(ext):
        return
